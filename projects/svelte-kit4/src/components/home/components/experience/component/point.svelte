...
<div data-group={details?.group} class={`experience__work flex items-start text-white relative ${details?.category ? 'ml-24 -mt-20 text-5 lg:text-6 pr-1 lg:pr-3' : 'text-4 lg:text-5'}`}>
  <!--left line-->
  {#if !details?.category}
  ...
  <div class="flex-1">
    ...
    {#if 'end' in details || 'team' in details}
      <div class="pl-4 leading-0 whitespace-nowrap mb-3">
        ...
        <span class="work__team leading-4 inline-block text-4 m-0 px-2 py-1.5 bg-white rounded-r" data-color={details?.team}>
          {$t(`team.${details?.team}`)}
        </span>
      </div>
    {/if}
