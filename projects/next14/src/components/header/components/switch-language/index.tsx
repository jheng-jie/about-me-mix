...
export default function SwitchLanguage({ className = '' } = {}) {
  ...
  return (
    <div onClick={() => setShow(show => !show)} onKeyUp={() => false} className={`${className} flex items-center justify-center cursor-pointer relative select-none`}>
      {/*current locale*/}
      ...
      {/*dropdown*/}
      <motion.div initial="hidden" animate={show ? 'visible' : 'hidden'} variants={dropdownVariants} className="absolute top-0 left-1/2 -translate-x-1/2 pt-12 md:pt-11 lg:pt-12 z-10 w-12">
        <div className="bg-white rounded-1.5 py-1 shadow-md">
          {/*locale list*/}
          {i18n.locales.map((locale: string) => (
            <button onClick={event => onLocaleSelect(event, locale)} key={locale} className="hover:bg-gray-200 transition-colors p-1.5">
