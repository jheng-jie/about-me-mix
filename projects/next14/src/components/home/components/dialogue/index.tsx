...
export default ({ index = 0 }: { index?: number } = {}) => {
  ...
  // line bg
  const bg = useMemo(() => Array.from({ length: 5 }).map((_, index) => <div key={index} className="dialogue__bg w-0 h-21vh -mb-1 bg-zinc-700 rounded-br-50px first:rounded-tr-50px" />), [])
