...
export default ({ index = 0 }: { index?: number } = {}) => {
  ...
  return useMemo(() => {
    ...
    const dialogue = Array.from({ length: 5 }).map((_, index) => (
      <p key={index} data-step={index + 1} className="coding__dialogue hidden opacity-0 origin-top lg:origin-left scale-80 bg-white rounded-3 shadow-lg px-3 lg:px-5 leading-10 lg:leading-16 inline-block">
        {t(`section.coding.message.${index + 1}`)}
      </p>
    ))

    return (
      <section className="h-400vh bg-gradient-to-b from-zinc-950 to-zinc-800">
        <div ref={container} className="coding h-100vh w-full px-2 pb-2 sticky top-0 flex flex-col items-center justify-start lg:justify-center">
          {/*dialogue*/}
          <div className="flex items-center justify-center flex-col lg:flex-row pt-18 lg:pt-5 pb-7 lg:pb-5 flex-shrink-0 w-full">
            {/*icon*/}
            <img src={`${process.env.MIX_ASSETS_URL}/avatar.png`} alt="" className="w-20 h-20 mb-3 lg:mb-0 lg:w-30 lg:h-30 shadow-lg rounded-full" />
            {/*message*/}
            <div className="flex-shrink-0 max-w-100 flex-none lg:flex-1 lg:ml-10 text-4 lg:text-5 relative">
              <p className="coding__dialogue origin-top lg:origin-left bg-white rounded-3 shadow-lg px-3 lg:px-5 leading-10 lg:leading-16 inline-block" data-step="0">
