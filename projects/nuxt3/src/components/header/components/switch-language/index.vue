...
<template>
  <div class="flex items-center justify-center cursor-pointer relative select-none" @keyup="() => false" @click="show = !show">
    <!--current locale-->
    ...
    <transition name="fade">
      <div v-show="show" class="absolute top-0 left-1/2 -translate-x-1/2 pt-12 md:pt-11 lg:pt-12 z-10 w-12">
        <div class="bg-white rounded-1.5 py-1 shadow-md">
          <!--locale list-->
          <button @click.prevent.stop="onSelectLocale(item as string)" v-for="item in locales" :key="item" class="hover:bg-gray-200 transition-colors p-1.5">
            <img :src="iconMap.get(item as string)" alt="" class="w-9 h-9" />
          </button>
        </div>
      </div>
    </transition>
